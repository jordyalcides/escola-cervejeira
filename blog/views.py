...
def index(request):
    posts = Post.objects.filter(data_de_publicação__lte=timezone.now()).order_by('data_de_publicação')
    curso = get_object_or_404(Curso)
...
def cervejas(request):
    cervejas = Cerveja.objects.filter(data_de_publicação__lte=timezone.now()).order_by('nome')
...
def cursos(request):
    cursos = Curso.objects.filter(data_de_publicação__lte=timezone.now()).order_by('nome')
